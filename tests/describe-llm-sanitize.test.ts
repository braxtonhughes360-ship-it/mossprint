...
describe('sanitizeItems (untrusted model output)', () => {
  ...
  it('preserves item_assumed only when the model sends true', () => {
    const [assumed, explicit, missing] = sanitizeItems({
      items: [
        validItem({ name: 'medium fries', item_assumed: true }),
        validItem({ name: 'big mac', item_assumed: false }),
        validItem({ name: 'salad' })
      ]
    })
    expect(assumed.item_assumed).toBe(true)
    expect(explicit.item_assumed).toBe(false)
    expect(missing.item_assumed).toBe(false)
  })
})
