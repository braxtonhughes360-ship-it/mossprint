...
describe('commitCaptureDraft', () => {
  ...
  it('writes income drafts positive with type income', () => {
    mockTransaction({ payeeName: 'paycheck', categoryId: null })
    const result = commitCaptureDraft({
      kind: 'money',
      money: {
        amountCents: 240_000,
    ...
    expect(result.message).toContain(' in')
    expect(createTransaction).toHaveBeenCalledWith(
      expect.objectContaining({ amountCents: 240_000, type: 'income' })
    )
  })
