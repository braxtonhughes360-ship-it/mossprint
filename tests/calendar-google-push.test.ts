...
describe('echo protection (push → pull reconcile)', () => {
  it('an identical echo reconciles without a write — no duplicate, not counted as updated', () => {
    ...
    expect(reconciled).toBe('unchanged')
  ...
  it('a changed echo reconciles with a write and counts as updated', () => {
    const id = insertEvent({ push_state: 'pending' })
    recordPushSuccess(db, id, 'gid-123')
    ...
    expect(reconciled).toBe('updated')
    expect(eventCount()).toBe(1)
    const row = getRow(id)
  ...
  it('falls back to the dashless MOSS UUID when the push crashed before recording the id', () => {
    ...
    // Recording the missing google_event_id is a real write, so this echo counts.
    expect(reconciled).toBe('updated')
    const row = getRow(id)
  ...
  it('a replayed cancellation (full re-sync) is a no-op write', () => {
    const id = insertEvent({ push_state: 'pushed', google_event_id: 'gid-789' })
    expect(reconcileGoogleCancellation(db, 'gid-789')).toBe(true)
    const afterFirst = getRow(id)

    expect(reconcileGoogleCancellation(db, 'gid-789')).toBe(true)
    expect(getRow(id).deleted_at).toBe(afterFirst.deleted_at)
    expect(getRow(id).updated_at).toBe(afterFirst.updated_at)
  })
})
