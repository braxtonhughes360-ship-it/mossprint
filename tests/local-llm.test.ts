...
// Controllable bundled-runtime state for the A1 resolution-policy matrix.
const runtimeMock = vi.hoisted(() => ({
  bundledReady: false,
  sidecarUrl: 'http://127.0.0.1:9876' as string | null
}))

vi.mock('../src/main/localRuntime', () => ({
  BUNDLED_MODEL: { tag: 'qwen3.5:built-in' },
  isBundledRuntimeAvailable: () => runtimeMock.bundledReady,
  isBundledModelReady: () => runtimeMock.bundledReady,
  sidecarBaseUrlForRouting: async () => (runtimeMock.bundledReady ? runtimeMock.sidecarUrl : null),
  ensureSidecarRunning: async () => (runtimeMock.bundledReady ? runtimeMock.sidecarUrl : null),
  getRuntimeStateForPanel: () => ({
    bundledAvailable: runtimeMock.bundledReady,
    consent: 'accepted',
    download: { status: 'ready', receivedBytes: 0, totalBytes: 0, error: null },
    source: 'none',
    warmCallMs: null
  }),
  noteSidecarUse: () => {},
...
describe('endpoint resolution policy (beta.5 A1 — bundled model first)', () => {
  const fetchMock = vi.fn()

  function ollamaHas(...models: string[]): void {
  ...
  beforeEach(() => {
    ...
    runtimeMock.bundledReady = true
    runtimeMock.sidecarUrl = 'http://127.0.0.1:9876'
  })

  afterEach(() => {
    vi.unstubAllGlobals()
    runtimeMock.bundledReady = false
  })

  it('ranks the bundled tag at the top of the preference order', () => {
    expect(modelRank('qwen3.5:built-in')).toBe(0)
    expect(modelRank('llama3.2:3b')).toBeGreaterThan(0)
    expect(modelRank('some-unknown:tag')).toBe(Number.POSITIVE_INFINITY)
  })

  it('bundled model beats a lower-ranked Ollama model (the beta.4 QA-10 case)', async () => {
    ollamaHas('llama3.2:latest', 'qwen2.5-coder:7b', 'nomic-embed-text:latest')
    expect(await probeOllama()).toEqual({ model: 'qwen3.5:built-in', error: null })
  })

  it('an Ollama model at the bundled class (qwen3.5+) keeps winning', async () => {
    ollamaHas('qwen3.5:4b', 'llama3.2:latest')
    expect(await probeOllama()).toEqual({ model: 'qwen3.5:4b', error: null })
  })

  it('the explicit engine preference overrides the bundled default', async () => {
    getSetting.mockImplementation((key: string) => {
      if (key === LOCALAI_ENGINE_SETTING) return { value: 'ollama' }
      if (key === LOCALAI_MODEL_SETTING) return { value: 'llama3.2' }
      return null
    })
    ollamaHas('llama3.2:latest', 'qwen2.5-coder:7b')
    expect(await probeOllama()).toEqual({ model: 'llama3.2:latest', error: null })
  })

  it('a stale beta.4 model pick WITHOUT the engine key does not beat the bundled default', async () => {
    // The operator case: llama3.2 selected in the old dropdown, bundled Qwen downloaded later.
    getSetting.mockImplementation((key: string) => {
      if (key === LOCALAI_MODEL_SETTING) return { value: 'llama3.2' }
      return null
    })
    ollamaHas('llama3.2:latest')
    expect(await probeOllama()).toEqual({ model: 'qwen3.5:built-in', error: null })
  })

  it('bundled model serves alone when Ollama is not running', async () => {
    fetchMock.mockRejectedValue(new Error('ECONNREFUSED'))
    expect(await probeOllama()).toEqual({ model: 'qwen3.5:built-in', error: null })
  })

  it('falls back to Ollama for this call while the sidecar is still warming', async () => {
    runtimeMock.sidecarUrl = null // cold start: routing budget expires
    ollamaHas('llama3.2:latest')
    expect(await probeOllama()).toEqual({ model: 'llama3.2:latest', error: null })
  })

  it('keeps today\'s Ollama behavior when the bundled model is not downloaded', async () => {
    runtimeMock.bundledReady = false
    ollamaHas('llama3.2:latest')
    expect(await probeOllama()).toEqual({ model: 'llama3.2:latest', error: null })
  })
