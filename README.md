...
Smart parsing (turn plain English into transactions, meals, and events) runs a small AI model **on your own computer** — MOSS includes it, with a one-time ~2.7GB download you approve on first use. It's happiest with 8 GB+ of RAM; everything else works without it, and if you already run [Ollama](https://ollama.com) MOSS will use that instead.
...
- **No telemetry, no analytics, no ads.** MOSS talks only to the services you connect yourself — your mail provider, your calendar, the feeds you pick — plus two small built-in lookups (a UV forecast for the dashboard, an update check on GitHub) that never carry your data.
