...
export function registerNotesHandlers(): void {
  ...
  // Attachments touch the filesystem, so they additionally gate on an unlocked profile.
  ipcMain.handle(IPC_CHANNELS.NOTES_LIST_ATTACHMENTS, (event, noteId: unknown) => {
    assertTrustedSender(event)
  ...
  ipcMain.handle(IPC_CHANNELS.NOTES_CREATE_ATTACHMENT, (event, input: unknown) => {
    assertTrustedSender(event)
    requireActiveProfileDatabase()
    if (!input || typeof input !== 'object') {
      throw new Error('Invalid attachment input')
    }
    const payload = input as CreateNoteAttachmentInput
    assertNonEmptyString(payload.noteId, 'noteId')
    ...
    return createNoteAttachment(payload)
  })

  ipcMain.handle(IPC_CHANNELS.NOTES_UPDATE_ATTACHMENT, (event, id: unknown, patch: unknown) => {
    assertTrustedSender(event)
    requireActiveProfileDatabase()
    assertNonEmptyString(id, 'id')
    if (!patch || typeof patch !== 'object') {
      throw new Error('Invalid attachment patch')
    }
    return updateNoteAttachment(id, patch as UpdateNoteAttachmentInput)
  })

  ipcMain.handle(IPC_CHANNELS.NOTES_UPDATE_SKETCH, (event, id: unknown, input: unknown) => {
    assertTrustedSender(event)
    requireActiveProfileDatabase()
    assertNonEmptyString(id, 'id')
    if (!input || typeof input !== 'object') {
      throw new Error('Invalid sketch input')
    }
    const payload = input as UpdateNoteSketchInput
    if (!(payload.bytes instanceof Uint8Array)) {
      throw new Error('bytes must be a Uint8Array')
    }
    return updateNoteSketch(id, payload)
  })

  ipcMain.handle(IPC_CHANNELS.NOTES_DELETE_ATTACHMENT, (event, id: unknown) => {
    assertTrustedSender(event)
    requireActiveProfileDatabase()
    assertNonEmptyString(id, 'id')
    return deleteNoteAttachment(id)
  })
