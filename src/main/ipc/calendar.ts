...
import {
  ...
  getCalendarEventPushInfo,
  getGoogleCalendarStatus,
  pushPendingGoogleEvents,
...
export function registerCalendarHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.CALENDAR_CREATE_EVENT, (event, input: unknown) => {
    assertTrustedSender(event)
    const record = createCalendarEvent(assertCreateInput(input))
    // Best-effort immediate push to the linked Google calendar — the event returns to the
    // renderer instantly; a failed push stays queued and retries on the next sync.
    void pushPendingGoogleEvents()
    return record
  })
  ...
  ipcMain.handle(IPC_CHANNELS.CALENDAR_UPDATE_EVENT, (event, id: unknown, patch: unknown) => {
    ...
    const record = updateCalendarEvent(id, assertUpdateInput(patch))
    void pushPendingGoogleEvents()
    return record
  })
  ...
  ipcMain.handle(IPC_CHANNELS.CALENDAR_DELETE_EVENT, async (event, id: unknown) => {
    ...
    const wasPushed = existing && !existing.sourceId
      ? getCalendarEventPushInfo(id)?.googleEventId != null
      : false

    deleteCalendarEvent(id)

    // MOSS-created event that MOSS itself pushed to Google: run the queued upstream delete
    // now so "delete in MOSS → gone in Google" is immediate. Never fires for events MOSS
    // didn't push (google_event_id is only ever set by a successful push).
    if (wasPushed) {
      const push = await pushPendingGoogleEvents()
      if (getCalendarEventPushInfo(id)?.pushState === 'pushed') {
        remoteDeleted = true
        remoteReason = undefined
      } else {
        remoteReason = push.reconnectRequired ? 'reconnect-required' : 'sync-error'
      }
    }
  ...
  ipcMain.handle(IPC_CHANNELS.CALENDAR_CREATE_CLASS_SCHEDULE, (event, input: unknown) => {
    assertTrustedSender(event)
    const result = createClassSchedule(assertClassScheduleInput(input))
    // A term of class meetings can be dozens of events — push in the background.
    void pushPendingGoogleEvents()
    return result
  })
