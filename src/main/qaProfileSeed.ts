...
import { createNote, createNoteFolder, createNoteTask, setNotePinned } from './notes'
...
async function seedNews(skipSync = false): Promise<void> {
  const feeds = [
...
export interface QaProfileSeedOptions {
  ...
  force?: boolean
  /**
   * Register news sources without hitting the network (newsFetch has no request
   * timeout, so a dead network hangs the seed forever). Harnesses that need
   * deterministic rows seed their own items afterwards.
   */
  skipNewsSync?: boolean
}

/**
 * Notes for the document model (R1): folders + notes across the folder rail,
 * pinned/checklist/plain variants, so Notes QA always has something to open.
 */
function seedNotes(): void {
  const recipes = createNoteFolder({ name: 'Recipes' })
  const trips = createNoteFolder({ name: 'Trips' })

  const soup = createNote({
    folderId: recipes.id,
    title: 'Lentil soup',
    body: 'Brown the onions first — do not rush this part.\nRed lentils, cumin, smoked paprika, lemon at the end.'
  })
  setNotePinned(soup.id, true)
  createNote({
    folderId: recipes.id,
    title: 'Sunday pancakes',
    body: 'Rest the batter 10 minutes. Butter, not oil.'
  })

  const packing = createNote({
    folderId: trips.id,
    title: 'Cabin packing list',
    isChecklistMode: true
  })
  createNoteTask({ noteId: packing.id, label: 'Headlamp' })
  createNoteTask({ noteId: packing.id, label: 'Rain shell' })
  createNoteTask({ noteId: packing.id, label: 'Board games' })

  // Default folder — proves un-foldered captures surface in the list too.
  createNote({ title: 'Gift ideas', body: 'Dad: wood plane. Sam: film for the camera.' })
}
...
export async function runQaProfileSeed(options: QaProfileSeedOptions = {}): Promise<void> {
  ...
  seedNutrition()
  seedNotes()
  await seedNews(options.skipNewsSync === true)
