...
export async function runHeadlessQa2Smoke(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      // W2e: Adjust is a quiet MossButton now — no bespoke class; find it by label.
      await exec(
        `Array.from(document.querySelectorAll('.calendar-quick-add-actions button'))
          .find((b) => b.textContent.trim() === 'Adjust').click()`
      )
    ...
    try {
      ...
      // W2i: starter chips are quiet MossButtons now — anchor on the row, not a chip class.
      await waitFor('.moss-setup-topic-row button', 'starter envelope chips')
      const chipOk = await exec<boolean>(
        `Array.from(document.querySelectorAll('.moss-setup-topic-row button')).some((b) => b.textContent.includes('Fun'))`
      )
      await exec(
        `Array.from(document.querySelectorAll('.moss-setup-topic-row button')).find((b) => b.textContent.includes('Fun')).click()`
      )
