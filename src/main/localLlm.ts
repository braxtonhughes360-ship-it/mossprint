...
export const LOCALAI_ENGINE_SETTING = LOCALAI_SETTING_KEYS.engine
...
export async function getLocalAiPanelState(): Promise<LocalAiPanelState> {
  ...
  // The probe's resolved endpoint is the truth the panel reports — not just
  // "is the sidecar process up" (it may be idle-stopped or still warming).
  if (masterEnabled && probeCache?.endpoint) {
    runtime.source = probeCache.endpoint.kind === 'ollama' ? 'ollama' : 'bundled'
  }
...
/** Explicit engine choice; anything but 'ollama' means bundled-first default. */
function readEnginePreference(): 'ollama' | null {
  try {
    return getSetting(LOCALAI_ENGINE_SETTING)?.value === 'ollama' ? 'ollama' : null
  } catch {
    return null
  }
}
...
/** Rank of a model tag in PREFERRED_MODELS (lower = better); Infinity when unlisted. */
export function modelRank(name: string): number {
  const idx = PREFERRED_MODELS.findIndex((prefix) => name.startsWith(prefix))
  return idx === -1 ? Number.POSITIVE_INFINITY : idx
}

/**
 * Resolve the active endpoint. Beta.5 A1 policy: the bundled model is the
 * product default once downloaded — a user Ollama only outranks it when it
 * has a model at-or-above the bundled class (qwen3.5+) or the user explicitly
 * picked an installed Ollama model in settings. Without the bundled model,
 * Ollama serves as before. Cached briefly so repeated calls stay snappy.
 */
async function resolveEndpoint(): Promise<{
  ...
  let ollama: LocalLlmEndpoint | null = null
  ...
  try {
    ...
    if (response.ok) {
      const parsed = (await response.json()) as { models?: Array<{ name?: string }> }
      installed = (parsed.models ?? [])
        .map((row) => (typeof row.name === 'string' ? row.name : ''))
        .filter(Boolean)
      const model = pickPreferredModel(installed)
      if (model) {
        ollama = { baseUrl: OLLAMA_BASE_URL, kind: 'ollama', model }
      } else {
        ollamaError = 'Ollama is running but has no models installed.'
      }
    } else {
  ...
  const bundledReady = isBundledRuntimeAvailable() && isBundledModelReady()

  // Only the explicit engine preference (written by the beta.5 panel) expresses
  // "use my Ollama over the bundled default". A stored localai.model alone does
  // NOT — beta.4 profiles picked a model in a dropdown that predates the bundled
  // choice, and honoring that as engine intent left the 2.7GB download unused.
  const prefersOllama = readEnginePreference() === 'ollama'

  const ollamaWins =
    ollama !== null &&
    (!bundledReady || prefersOllama || modelRank(ollama.model) <= modelRank(BUNDLED_MODEL.tag))
  ...
  if (ollamaWins) {
    ...
    if (baseUrl) {
      ...
      // Sidecar still loading the model. Serve this window from Ollama when it
      // exists (never block capture on a cold start), and re-probe soon so the
      // warmed sidecar takes over — don't pin the fallback for a minute.
      endpoint = ollama
      ttlMs = PROBE_CACHE_WARMING_MS
    }
  }

  // Ollama's failure detail only matters when it's the reason nothing works.
  const error = endpoint ? null : ollamaError
