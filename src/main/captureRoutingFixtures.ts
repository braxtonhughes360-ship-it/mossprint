...
import { createCategory, listTransactions } from './money'
...
export async function runCaptureRoutingFixtures(): Promise<{
  ...
  const incomeRow = listTransactions(50).find((row) => row.amountCents === 240_000)
  if (
    committedIncome.status !== 'logged' ||
    !committedIncome.message.includes(' in') ||
    listTransactions(50).length !== beforeCommits + 2 ||
    !incomeRow ||
    incomeRow.type !== 'income'
  ) {
    failures.push('income confirm did not write one positive income transaction')
  }
  checks += 1
  try {
    commitCaptureDraft({
    ...
    if (listTransactions(50).length !== beforeCommits + 2) {
