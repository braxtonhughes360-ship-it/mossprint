...
/** Closing the window quits MOSS — no dock/minimize linger (macOS + Windows). */
...
const AUDIT_ONLY_SHOTS = [
  ...
  { route: '/notes', filename: 'notes', readySelector: '.moss-arrival-notes' },
...
async function runHeadlessReadmeShots(): Promise<void> {
  try {
    ...
    await runQaProfileSeed({ quitApp: false, force: true })
...
async function runHeadlessDescribeSmoke(): Promise<void> {
  ...
  try {
    ...
    const results: Array<{
      text: string
      items: Array<{ label: string; source: string; kcal: number }>
    }> = []
