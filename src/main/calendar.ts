...
import { readGoogleToken } from './calendarCredentials'

const EVENT_KIND_SET = new Set<string>(CALENDAR_EVENT_KINDS)

/**
 * MOSS-created events are queued for push to Google only when a linked Google source exists
 * at creation time — connecting Google later must never surprise-upload the local history.
 */
function googlePushTargetConnected(): boolean {
  const row = getDb()
    .prepare(
      `
  ...
  return Boolean(row && readGoogleToken(row.id))
}
...
export function createCalendarEvent(input: CreateCalendarEventInput): CalendarEventRecord {
  ...
  getDb()
    ...
    .run({
      ...
      kind,
      pushState: googlePushTargetConnected() ? 'pending' : null,
      createdAt,
      updatedAt
    })
  ...
  getDb()
    .prepare(
      ...
      UPDATE calendar_events SET
        ...
        push_state = CASE WHEN push_state IS NULL THEN NULL ELSE 'pending' END,
  ...
  const result = getDb()
    .prepare(
      ...
      SET deleted_at = @updatedAt,
          updated_at = @updatedAt,
          push_state = CASE
            WHEN google_event_id IS NOT NULL THEN 'pending'
            ELSE NULL
          END
