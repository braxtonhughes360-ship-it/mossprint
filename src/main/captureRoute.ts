...
import { hasMoneyIncomeHint } from '@shared/moneyDescribeParse'
import {
  describeCaptureIntent,
  MAX_AMOUNT_CENTS,
  type SanitizedCaptureIntent
} from './captureIntentLlm'
import { createPaycheck, createTransaction } from './money'
...
function draftMoneyConfirm(intent: SanitizedCaptureIntent): CaptureSubmitResult | null {
  ...
  // Income has no envelope: it lands in the budget as a paycheck (A2), and the
  // preview says so \u2014 the operator's "$1,400 went nowhere" must never recur.
  const envelopeSuffix =
    direction === 'income'
      ? ' \u00b7 adds to budget'
      : intent.moneyCategoryId
...
export async function previewCaptureText(
  ...
  switch (classified.kind) {
    case 'money':
      return draftDeterministicMoneyConfirm(
        classified.amountCents,
        classified.merchant,
        hasMoneyIncomeHint(text) ? 'income' : 'expense'
      )
    case 'nutrition':
      return draftNutritionPlate(text)
    case 'calendar': {
...
/** Confirm draft for a deterministically-classified money line (no LLM fields). */
function draftDeterministicMoneyConfirm(
  amountCents: number,
  merchant: string,
...
export async function routeCaptureText(rawText: string): Promise<CaptureSubmitResult> {
  ...
  switch (classified.kind) {
    case 'money':
      // A2: income-hinted lines ("$1400 paycheck") confirm as income instead of
      // instant-writing an expense — income is rarer and higher-stakes than a
      // coffee, and the paycheck write lands in the budget, not the ledger.
      if (hasMoneyIncomeHint(text)) {
        return draftDeterministicMoneyConfirm(classified.amountCents, classified.merchant, 'income')
      }
...
export function commitCaptureDraft(input: CaptureCommitInput): CaptureLoggedResult {
  switch (input.kind) {
    case 'money': {
      ...
      if (direction === 'income') {
        // A2: income is a PAYCHECK \u2014 the budget's income representation
        // (matches moneyV2 schedule posting). A ledger income row would be
        // invisible to "to assign" / income-vs-plan, which is exactly the
        // beta.4 QA finding this fixes. Paychecks carry no envelope.
        createPaycheck({
          label: safeMerchant || 'Income',
          amountCents: Math.abs(Math.round(amountCents)),
          receivedAt: dayKeyToIso(draftDateKey ?? dateKey())
        })
        return {
          status: 'logged',
          kind: 'money',
          message: `Logged ${formatCaptureAmount(amountCents)} in${safeMerchant ? ` \u2014 ${safeMerchant}` : ''} \u00b7 ${dayLabel} \u00b7 added to budget`
        }
      }
      ...
      return {
        status: 'logged',
        kind: 'money',
        message: `Logged ${formatCaptureAmount(amountCents)} out${label ? ` \u2014 ${label}` : ''} \u00b7 ${dayLabel}${envelope}`
      }
    }
    case 'calendar': {
