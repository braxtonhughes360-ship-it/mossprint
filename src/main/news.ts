  ...
  try {
    ...
    if (!notModified) {
      for (const entry of entries) {
        const existing = getDb()
          .prepare(
            `SELECT id, title, url, summary, image_url
             FROM news_items WHERE source_id = ? AND external_id = ?`
          )
          .get(sourceId, entry.externalId) as
          | { id: string; title: string; url: string; summary: string; image_url: string }
          | undefined

        if (existing) {
          // published_at stays out of the comparison: the AP fallback fabricates it as
          // "now" on every fetch, so comparing it would rewrite (and re-freshen) every
          // AP story each sync. Unchanged content keeps its first-seen timestamp.
          const changed =
            existing.title !== entry.title ||
            existing.url !== entry.url ||
            existing.summary !== entry.summary ||
            existing.image_url !== entry.imageUrl
          if (changed) {
            getDb()
              .prepare(
