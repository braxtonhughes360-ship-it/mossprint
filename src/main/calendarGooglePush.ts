...
/**
 * Reconciles a pulled Google event against a MOSS-pushed local row. Returns true when a match
 * was found and updated (the caller must then skip the source-linked upsert — echo protection).
 * The Google copy wins: local fields are overwritten (last-writer-wins, Google on pull).
 */
...
function findPushedLocalRow(db: SqliteDatabase, googleEventId: string): { id: string } | null {
  ...
  const row = db
    ...
    .get({ googleEventId }) as { id: string } | undefined
  return row ?? null
}
