...
function verifyNotesTables(database: SqliteDatabase): boolean {
  ...
  const run = database.transaction(() => {
    ...
    database
      .prepare(
        `INSERT INTO notes (
           ...
           @id, @folderId, 'Health check note', 'Body text', 1, 1, '["maintenance"]', @now, @now
         )`
      )
    ...
    const note = database
      .prepare('SELECT title, tags FROM notes WHERE id = ?')
      .get(noteId) as { title: string; tags: string } | undefined
...
export function runHealthCheck(): DatabaseHealthResult {
  ...
  return {
    ...
    databasePath,
    message: ok
      ? 'SQLite settings + money + nutrition + calendar + news + mail + notes + goals tables round-trip succeeded.'
      : readBack?.value !== token
