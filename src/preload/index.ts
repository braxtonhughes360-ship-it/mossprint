...
const headlessLoadingDelayMs = Math.max(
  0,
  Number.parseInt(process.env.MOSS_HEADLESS_LOADING_DELAY_MS ?? '0', 10) || 0
)

async function invokeProgressivePreview(channel: string, ...args: unknown[]): Promise<unknown> {
  if (headlessLoadingDelayMs > 0) {
    await new Promise((resolve) => setTimeout(resolve, headlessLoadingDelayMs))
  }
...
const mossBridge = {
  ...
  data: {
    getOverview: () => ipcRenderer.invoke(IPC_CHANNELS.DATA_GET_OVERVIEW),
    showInFolder: () => ipcRenderer.invoke(IPC_CHANNELS.DATA_SHOW_IN_FOLDER),
    pickMoveTarget: () => ipcRenderer.invoke(IPC_CHANNELS.DATA_PICK_MOVE_TARGET),
    moveFolder: (targetPath: string) =>
      ipcRenderer.invoke(IPC_CHANNELS.DATA_MOVE_FOLDER, targetPath)
  },
  profiles: {
  ...
  money: {
    ...
    getDoorSnapshot: (periodKey?: string) =>
      invokeProgressivePreview(IPC_CHANNELS.MONEY_GET_DOOR_SNAPSHOT, periodKey),
  ...
  nutrition: {
    ...
    getDoorSnapshot: (dateKey?: string) =>
      invokeProgressivePreview(IPC_CHANNELS.NUTRITION_GET_DOOR_SNAPSHOT, dateKey),
    ...
    describeMeal: (input: unknown) =>
      invokeProgressivePreview(IPC_CHANNELS.NUTRITION_DESCRIBE_MEAL, input),
  ...
  calendar: {
    ...
    getDoorSnapshot: () => invokeProgressivePreview(IPC_CHANNELS.CALENDAR_GET_DOOR_SNAPSHOT)
  },
  news: {
    ...
    getBriefing: (options?: unknown) =>
      invokeProgressivePreview(IPC_CHANNELS.NEWS_GET_BRIEFING, options),
  ...
  mail: {
    getStatus: () => invokeProgressivePreview(IPC_CHANNELS.MAIL_GET_STATUS),
    setGoogleOAuth: (clientId: string, clientSecret: string) =>
    ...
    listMessages: (options?: unknown) =>
      invokeProgressivePreview(IPC_CHANNELS.MAIL_LIST_MESSAGES, options),
    countMessages: (options?: unknown) =>
      ipcRenderer.invoke(IPC_CHANNELS.MAIL_COUNT_MESSAGES, options),
    getMessage: (messageId: string) =>
      invokeProgressivePreview(IPC_CHANNELS.MAIL_GET_MESSAGE, messageId),
    getDoorSnapshot: () => invokeProgressivePreview(IPC_CHANNELS.MAIL_GET_DOOR_SNAPSHOT),
    ...
    aiDraftReply: (messageId: string, instruction?: string) =>
      ipcRenderer.invoke(IPC_CHANNELS.MAIL_AI_DRAFT_REPLY, messageId, instruction),
    listDrafts: (accountId?: string) =>
      invokeProgressivePreview(IPC_CHANNELS.MAIL_LIST_DRAFTS, accountId),
    getDraft: (draftId: string) => ipcRenderer.invoke(IPC_CHANNELS.MAIL_GET_DRAFT, draftId),
    saveDraft: (input: unknown) => ipcRenderer.invoke(IPC_CHANNELS.MAIL_SAVE_DRAFT, input),
    deleteDraft: (draftId: string) => ipcRenderer.invoke(IPC_CHANNELS.MAIL_DELETE_DRAFT, draftId),
    countDrafts: (accountId?: string) =>
      ipcRenderer.invoke(IPC_CHANNELS.MAIL_COUNT_DRAFTS, accountId)
  },
  notes: {
    ...
    getDoorSnapshot: () => invokeProgressivePreview(IPC_CHANNELS.NOTES_GET_DOOR_SNAPSHOT),
    listAttachments: (noteId: string) =>
      ipcRenderer.invoke(IPC_CHANNELS.NOTES_LIST_ATTACHMENTS, noteId),
    createAttachment: (input: unknown) =>
      ipcRenderer.invoke(IPC_CHANNELS.NOTES_CREATE_ATTACHMENT, input),
    updateAttachment: (id: string, patch: unknown) =>
      ipcRenderer.invoke(IPC_CHANNELS.NOTES_UPDATE_ATTACHMENT, id, patch),
    updateSketch: (id: string, input: unknown) =>
      ipcRenderer.invoke(IPC_CHANNELS.NOTES_UPDATE_SKETCH, id, input),
    deleteAttachment: (id: string) =>
      ipcRenderer.invoke(IPC_CHANNELS.NOTES_DELETE_ATTACHMENT, id)
  },
  goals: {
  ...
  capture: {
    ...
    onShown: (callback: () => void) => {
      const handler = (): void => callback()
      ipcRenderer.on(CAPTURE_SHOWN_EVENT, handler)
      return () => ipcRenderer.removeListener(CAPTURE_SHOWN_EVENT, handler)
    }
  },
  localai: {
