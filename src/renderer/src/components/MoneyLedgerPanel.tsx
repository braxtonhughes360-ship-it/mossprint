...
import { MossListStagger, MossListStaggerItem } from './MossListStagger'
...
export function MoneyLedgerPanel({
  ...
  const patchFilter = useCallback((patch: Partial<LedgerFilter>): void => {
    setFilter((prev) => ({ ...prev, ...patch }))
  }, [])

  const toggleTransaction = useCallback((transactionId: string): void => {
    setExpandedId((id) => (id === transactionId ? null : transactionId))
  }, [])

  const handleDeleted = useCallback((token: string, label: string): void => {
    setExpandedId(null)
    setUndo({ token, label })
  }, [])
      ...
      <section className="money-instrument-panel money-register" aria-label="Ledger">
        ...
        {showFilters && (
          <div className="money-ledger-filterbar money-ledger-filterbar--more">
            ...
            />
            <MossDateField
              className="money-date-field--register"
              value={filter.from ?? ''}
        ...
        {undo && (
          <div className="money-ledger-undo" role="status">
            ...
            <div className="money-ledger-undo-actions">
              <MossButton type="button" size="sm" onClick={undoDelete}>
                Undo
              </MossButton>
              <MossButton
                type="button"
        ...
        <div className="money-ledger-register">
          ...
          <div className="money-register-scroll">
            <MossListStagger as="ul" className="money-ledger-tape">
              {displayed.length === 0 && (
                <MossListStaggerItem as="li" className="money-instrument-empty">
