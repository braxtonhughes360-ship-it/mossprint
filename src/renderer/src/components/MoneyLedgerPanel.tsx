...
import { MoneyLedgerRow } from './MoneyLedgerRow'
...
export function MoneyLedgerPanel({
  ...
  const displayed = useMemo(
    () =>
      filterTransactions(accountScoped, filter).sort(
    ...
    if (entryType === 'transfer') {
      ...
      void onMutate(async () => {
        await window.moss.money.createTransfer({
          ...
          occurredAt: periodMidpointIso(periodKey),
          ...
          <div className="money-register-scroll">
            ...
            {displayed.map((txn) => (
              <MoneyLedgerRow
                key={txn.id}
                txn={txn}
                balanceCents={balanceById.get(txn.id) ?? 0}
                ...
                onToggle={() => setExpandedId((id) => (id === txn.id ? null : txn.id))}
                onMutate={onMutate}
        ...
        >
          <div className="money-register-entry-top">
            <div className="money-kind-toggle money-kind-toggle--register">
              <button
                type="button"
                className={['money-kind', entryType === 'expense' ? 'money-kind--active' : ''].join(' ')}
              ...
              <button
                type="button"
                className={['money-kind', entryType === 'income' ? 'money-kind--active' : ''].join(' ')}
              ...
              {accounts.length >= 2 && (
                <button
                  type="button"
                  className={['money-kind', entryType === 'transfer' ? 'money-kind--active' : ''].join(' ')}
              ...
              <button
                type="button"
                className={['money-kind', entryType === 'adjustment' ? 'money-kind--active' : ''].join(' ')}
