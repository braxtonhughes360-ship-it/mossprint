  ...
  return (
    <MossCard className="settings-card">
                ...
                <MailAccountRow
                  ...
                  onSync={() =>
                    void run(async () => {
                      ...
                      setFlash(
                        result.error
                          ? 'Refresh issue — showing the last good copy'
                          : `Updated · ${result.imported} new`
                      )
                    })
                  }
                  onDisconnect={() =>
      ...
      <p className="preference-hint">
        ...
        to read and reply. Refresh brings in the last 90 days (up to 1,500 messages per account).
      </p>
    </MossCard>
  )
}
    ...
    <li className="mail-account-row">
      <div className="mail-account-info">
        ...
        <span className="mail-account-status nutrition-mono">
          ...
          {' · '}
          {account.stale
            ? 'Last refresh failed — try again'
            : account.lastSyncAt
              ? `Updated ${new Date(account.lastSyncAt).toLocaleString()}`
              : 'Ready to refresh'}
        </span>
      </div>
      <MossToolbar label={`Actions for ${account.email}`} className="calendar-settings-actions">
        <MossToolbar.Group label="Account">
        <MossButton
          disabled={busy}
          onClick={onSync}
        >
