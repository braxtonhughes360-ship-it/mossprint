...
function NutritionReviewPlateSkeleton(): React.JSX.Element {
  return (
    <div className="nutrition-review-plate-backdrop" role="presentation">
      <section
        className="nutrition-review-plate nutrition-review-plate--skeleton"
        aria-busy="true"
        aria-label="Preparing meal review"
      >
        <header className="nutrition-review-plate-head">
          <div className="nutrition-review-plate-skeleton-heading">
            <MossSkeleton width="28%" height="0.625rem" />
            <MossSkeleton width="58%" height="1rem" />
          </div>
        </header>
        <div className="nutrition-review-plate-list">
          {Array.from({ length: 2 }, (_, index) => (
            <div className="nutrition-review-plate-row nutrition-review-plate-skeleton-row" key={index}>
              <MossSkeleton width={index === 0 ? '64%' : '48%'} />
              <MossSkeleton variant="block" height="2.5rem" />
              <MossSkeleton width="44%" height="0.625rem" />
            </div>
          ))}
        </div>
        <footer className="nutrition-review-plate-foot">
          <MossSkeleton width="32%" />
          <MossSkeleton variant="block" width="7rem" height="2.25rem" />
        </footer>
      </section>
    </div>
  )
      ...
      >
        <header className="nutrition-review-plate-head">
          ...
          <MossButton
            ...
            aria-label="Close"
          >
            <span className="nutrition-review-plate-close-glyph" aria-hidden>
              ×
            </span>
          </MossButton>
        </header>
        ...
        <footer className="nutrition-review-plate-foot">
          ...
          <div className="nutrition-review-plate-actions">
            <MossButton variant="quiet" onClick={onClose}>
              Cancel
            </MossButton>
            <MossButton
      ...
      >
        ...
        <form className="nutrition-manual-form" onSubmit={(event) => void handleSubmit(event)}>
          ...
          <footer className="nutrition-review-plate-foot nutrition-manual-foot">
            ...
            <MossButton type="submit" disabled={busy || !label.trim() || !kcal}>
...
export function NutritionDescribeField({
  ...
  return (
    ...
    >
      <MossToolbar className="nutrition-add-toolbar" label="Food logging actions">
        ...
        <MossToolbar.Group label="Entry method">
          <MossButton variant="quiet" size="sm" disabled={busy} onClick={onManualOpen}>
            Manual entry
          </MossButton>
        </MossToolbar.Group>
      </MossToolbar>
      ...
      <p className="nutrition-describe-hint">
        {llmModel ? (
          <>
            Smart parsing is ready and runs on this computer.
          </>
        ) : (
          <>
      ...
      {resolving ? <NutritionReviewPlateSkeleton /> : null}
    </section>
  )
}
