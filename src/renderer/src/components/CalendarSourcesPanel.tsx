    ...
    <MossCard className="settings-card">
      <header className="settings-card-head">
        ...
        <p className="settings-card-copy">
          Connect Google or school calendars. Events are stored on this computer — nothing is
          shared anywhere.
        </p>
      </header>
      ...
      <div className="settings-stack">
        <div className="settings-subsection calendar-google-panel">
          ...
          >
            <input
              ...
              placeholder="Paste Google’s secret calendar link"
              value={googleIcsUrl}
        ...
        <div className="settings-subsection calendar-google-panel">
          ...
          {googleConnected && googleSource ? (
            <div className="calendar-google-connected">
              ...
              <p className="preference-hint">
                {googleSource.stale
                  ? 'Last refresh failed — tap Refresh.'
                  : googleSource.lastSyncAt
                    ? `Last updated ${new Date(googleSource.lastSyncAt).toLocaleString()}`
                    : 'Ready to refresh'}
              </p>
              {!googleWriteCapable && (
                <p className="preference-hint">
              ...
              {googleWriteCapable && googlePendingPush > 0 && (
                <p className="preference-hint">
                  ...
                  to reach Google — Refresh tries again.
                </p>
              )}
              <MossToolbar label="Connected Google Calendar actions">
                <MossToolbar.Group label="Google Calendar">
                  <MossButton
                    type="button"
                    disabled={busy}
                    onClick={() =>
                      void run(async () => {
                        ...
                        const pushedNote = result.pushed
                          ? ` · ${result.pushed} sent to Google`
                          : ''
                        const pushErrorNote = result.pushErrors
                          ? ` · ${result.pushErrors} failed to send (will retry)`
                          : ''
                        setFlash(
                          `Updated · ${result.imported} new · ${result.updated} changed${pushedNote}${pushErrorNote}`
                        )
                      })
                    }
                  >
            ...
            <div className="calendar-google-connect-flow">
              ...
              <MossButton
                type="button"
                className="calendar-settings-action--wide"
                disabled={busy || !bridgeReady}
                onClick={() =>
                  void run(async () => {
        ...
        <div className="settings-subsection calendar-academics-toggle">
          <MossCheckbox
            className="calendar-academics-toggle-row"
            checked={academicsEnabled}
            onChange={(event) =>
              setPreferences({
                modules: {
                  calendar: {
                    ...preferences.modules.calendar,
                    academicsEnabled: event.target.checked
                  }
                }
              })
            }
        ...
        <div className="settings-subsection">
          ...
          <MossToolbar label="Calendar file actions">
            <MossToolbar.Group label="Import">
              <MossButton
                type="button"
                variant="quiet"
                disabled={busy || !bridgeReady}
                ...
                <span className="calendar-settings-source-meta">
                  {!source.enabled
                    ...
                    : source.stale
                      ? 'Stale'
                      : source.lastSyncAt
                        ? `Updated ${new Date(source.lastSyncAt).toLocaleString()}`
                        : 'Not updated yet'}
                </span>
                <MossButton
                  type="button"
