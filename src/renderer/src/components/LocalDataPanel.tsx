...
export function LocalDataPanel(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) {
          setLoadState('error')
          setError(err instanceof Error ? err.message : 'Could not check your data')
        }
      }
    })()
  ...
  return (
    <div className="memory-panel mt-6">
      ...
      <MossToolbar label="Storage verification actions" className="memory-actions mt-5">
        <MossToolbar.Group label="Verification">
          <MossButton
            busy={running}
            busyLabel="Verifying…"
          ...
          <MossButton tone="neutral" onClick={() => void refreshPing()}>
            Refresh
          </MossButton>
        </MossToolbar.Group>
      </MossToolbar>
...
function YourDataSection(): React.JSX.Element {
  ...
  return (
    <div className="mt-8">
      ...
      <MossToolbar label="Data folder actions" className="memory-actions mt-5">
        <MossToolbar.Group label="Location">
        {needsReload && (
          <MossButton onClick={() => window.location.reload()}>
            Reload MOSS
          </MossButton>
        )}
        <MossButton
