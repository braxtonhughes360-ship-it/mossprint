  ...
  async function syncFeeds(): Promise<void> {
    ...
    try {
      ...
      if (imported + updated > 0) {
        setFlash(`Synced · ${imported} new · ${updated} updated`)
      } else if (errors.length === 0) {
        setFlash('Synced — feeds match source')
      }
      await load()
    } catch (err) {
  ...
  function emptyCopy(): { title: string; copy: string } {
    if (items.length === 0) {
      return {
        ...
        copy: 'Add sources in Setup or Settings → News, then Sync now.'
      }
    }
    if (filter === 'unread') {
    ...
    if (typeof filter === 'object' && 'category' in filter) {
      return {
        ...
        copy: 'Try Sync now, or pick another filter above.'
      }
    }
    if (typeof filter === 'object') {
      const name = sources.find((s) => s.id === filter.sourceId)?.title ?? 'this source'
      return { title: `Nothing from ${name}`, copy: 'Try Sync now, or pick another source above.' }
    }
    return { title: 'No headlines yet', copy: 'Sync now to pull the latest.' }
  }
    ...
    <section className="news-reader-panel">
          ...
          {visibleSourceChips.map(({ source, unread, hasError }) => {
            ...
            return (
              <button
                ...
                title={hasError ? `Last sync failed: ${source.lastError}` : source.title}
              >
                {hasError && (
                  <span className="news-reader-chip-alert" aria-label="Sync error" />
                )}
                <NewsSourceLogo
      ...
      {!error && staleSources.length > 0 && (
        <p className="news-reader-health" role="status">
          ...
          last-good headlines. Sync now to retry.
        </p>
      )}
      {activeSourceError && (
        <p className="news-reader-health" role="status">
          Last sync failed: {activeSourceError}
        </p>
      )}
