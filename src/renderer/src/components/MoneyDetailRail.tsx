import type { MoneyBudgetOverview, ScheduleRecord, TransactionRecord } from '@shared/money'
import {
  formatMoneyCents,
  formatPeriodLabel,
  isScheduleDue,
...
interface MoneyDetailRailProps {
  ...
  /** Scheduled bills/income for the Upcoming section (QA2-09). */
...
/** Schedule nextDate is a day key — format via parts so UTC parsing can't shift the day. */
function formatScheduleDate(dayKey: string): string {
  ...
  return (
    <aside className="money-detail-rail" aria-label="Money ledger">
      ...
      <div className="money-detail-rail-inner">
        ...
        {upcoming.length > 0 && (
          <section className="money-detail-rail-section" aria-label="Upcoming scheduled">
            <p className="money-detail-rail-section-label">Upcoming</p>
            <ul className="money-detail-rail-tape">
              {upcoming.map((schedule) => {
                const due = isScheduleDue(schedule)
                const signed = scheduleSignedAmountCents(schedule)
                return (
                  <li key={schedule.id} className="money-detail-rail-tape-row">
                    <span
                      className={[
                        'money-detail-rail-tape-date',
                        'money-mono',
                        due ? 'money-detail-rail-tape-date--due' : ''
                      ]
                        .filter(Boolean)
                        .join(' ')}
                    >
                      {due ? 'Due' : formatScheduleDate(schedule.nextDate)}
                    </span>
                    <div className="money-detail-rail-tape-main">
                      <span className="money-detail-rail-tape-memo">{schedule.label}</span>
                      <span
                        className={[
