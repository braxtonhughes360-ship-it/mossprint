  ...
  const syncSources = useCallback(async () => {
    ...
    try {
      ...
      if (result.results.length > 0) {
        ...
        if (imported > 0) {
          setFlash(`Updated · ${imported} new event${imported === 1 ? '' : 's'}`)
          ...
          setFlash('Updated — everything is current')
        }
      }
      await queryClient.invalidateQueries({ queryKey: ['calendar'] })
    } catch (err) {
      setMutationError(err instanceof Error ? err.message : 'Calendar refresh failed')
    } finally {
      setSyncing(false)
    }
  }, [queryClient])
  ...
  return (
    ...
    >
                ...
                <button
                  ...
                  aria-label={
                    syncing
                      ...
                      : `Refresh now — ${staleCount} source${staleCount === 1 ? '' : 's'} need attention`
                  }
                  title={
                    syncing
                      ? 'Refreshing calendars…'
                      : `${staleCount} source${staleCount === 1 ? '' : 's'} need attention — refresh now`
                  }
      ...
      <div className="moss-arrival-body calendar-arrival-body">
        ...
        <MossPanelTransition transitionKey={view}>
          {view === 'week' && (
            <>
              {glance && <CalendarWeekStrip glance={glance} />}
              <CalendarWeekPanel
