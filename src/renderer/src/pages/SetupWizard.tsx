...
export function SetupWizard(): React.JSX.Element {
  ...
  // Finish links to data controls — create the profile a moment early so they are ready.
  useEffect(() => {
  ...
  async function handleNewsSave(): Promise<void> {
    ...
    const ok = await runTask(async () => {
      ...
      setNewsNote(
        `${feeds.length} source${feeds.length > 1 ? 's' : ''} added — fetching headlines in the background`
      )
        ...
        <div className="moss-setup-body">
          ...
          {step === STEP_PREFS && (
            <section className="moss-setup-step">
              ...
              />
              <div className="moss-setup-check moss-setup-check--solo">
                <MossCheckbox
                  label="Keep MOSS in the menu bar"
                  description="When this is on, closing the window keeps MOSS running in your menu bar so quick capture stays one click away. When it's off, closing the window quits MOSS."
          ...
          {step === STEP_MODULES && (
            <section className="moss-setup-step">
              ...
              <ul className="moss-setup-checklist">
                {MODULE_ROWS.map((row) => (
                  <li key={row.id}>
                    <div className="moss-setup-check">
                      <MossCheckbox
                        label={row.label}
                    ...
                    {row.id === 'money' && modules.money.enabled && (
                      <div className="moss-setup-check moss-setup-check--nested">
                        <MossCheckbox
                          label="Track investments too"
                          description="Optional 401k / brokerage balance snapshots, alongside your budget."
          ...
          {step === STEP_MONEY && (
            <section className="moss-setup-step">
              ...
              >
                ...
                <MossButton
                  type="submit"
                  variant={addedEnvelopes.length === 0 ? 'primary' : 'quiet'}
                  size="sm"
          ...
          {step === STEP_CALENDAR && (
            <section className="moss-setup-step">
              ...
              <MossCheckbox
                className="moss-setup-academics-toggle"
                label="I'm a student — add classes, exams, and assignments"
        ...
        <footer className="moss-setup-foot">
          ...
          <div className="moss-setup-foot-actions">
            {(showSkipSetup || isModuleConfigStep) && (
              <MossButton
                variant="quiet"
                className="moss-setup-button--skip"
                onClick={skipStep}
            ...
            >
              {step === STEP_FINISH ? 'Open MOSS' : isModuleConfigStep ? 'Continue' : 'Next'}
            </MossButton>
          </div>
        </footer>
      </div>
