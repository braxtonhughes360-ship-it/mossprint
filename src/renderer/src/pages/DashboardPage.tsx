...
export function DashboardPage(): React.JSX.Element {
  ...
  const { snapshot: moneySnapshot, refresh: refreshMoneySnapshot } = useMoneyDoorSnapshot()
  const { snapshot: nutritionSnapshot, refresh: refreshNutritionSnapshot } =
    useNutritionDoorSnapshot()
  const { snapshot: calendarSnapshot, refresh: refreshCalendarSnapshot } =
    useCalendarDoorSnapshot()
  const { snapshot: inboxSnapshot, refresh: refreshInboxSnapshot } = useInboxDoorSnapshot()
  const { snapshot: notesSnapshot, refresh: refreshNotesSnapshot } = useNotesDoorSnapshot()
  const {
