...
export function useCaptureFlow({
  ...
  const finishLogged = useCallback(
    (message: string, kind: CaptureKind): void => {
      onLogged?.(kind)
      setLastLoggedKind(kind)
      // Clear the logged text right away: 'done' keeps the confirmation line
      // on screen, but the input must be ready for the next thought — a bar
      // still holding the old sentence reads as stuck.
      setText('')
      if (variant === 'window') {
  ...
  const handleTextChange = useCallback(
    (value: string): void => {
      ...
      // Typing after a log starts the next capture — and cancels the window
      // variant's fade-out timers so it doesn't hide mid-thought.
      if (phase.name === 'done') {
        clearTimers()
        setLastLoggedKind(null)
        setPhase({ name: 'idle' })
