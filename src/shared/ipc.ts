...
import type { CaptureCommitInput, CaptureLoggedResult, CaptureSubmitResult } from './capture'
import type { LocalAiDownloadState, LocalAiPanelState, LocalAiSurface } from './localai'
...
export const IPC_CHANNELS = {
  ...
  DB_PING: 'db:ping',
  DATA_GET_OVERVIEW: 'data:get-overview',
  DATA_SHOW_IN_FOLDER: 'data:show-in-folder',
  DATA_PICK_MOVE_TARGET: 'data:pick-move-target',
  DATA_MOVE_FOLDER: 'data:move-folder',
  ...
  SHELL_GET_APP_SETTINGS: 'shell:get-app-settings',
  SHELL_SET_APP_SETTINGS: 'shell:set-app-settings',
  ...
  MAIL_SEND: 'mail:send',
  MAIL_AI_DRAFT_REPLY: 'mail:ai-draft-reply',
  MAIL_LIST_DRAFTS: 'mail:list-drafts',
  MAIL_GET_DRAFT: 'mail:get-draft',
  MAIL_SAVE_DRAFT: 'mail:save-draft',
  MAIL_DELETE_DRAFT: 'mail:delete-draft',
  MAIL_COUNT_DRAFTS: 'mail:count-drafts',
  ...
  NOTES_LIST_ATTACHMENTS: 'notes:list-attachments',
  NOTES_CREATE_ATTACHMENT: 'notes:create-attachment',
  NOTES_UPDATE_ATTACHMENT: 'notes:update-attachment',
  NOTES_UPDATE_SKETCH: 'notes:update-sketch',
  NOTES_DELETE_ATTACHMENT: 'notes:delete-attachment',
  ...
  LOCALAI_DESCRIBE_PREVIEW: 'localai:describe-preview',
  LOCALAI_WARM: 'localai:warm',
  LOCALAI_GET_STATE: 'localai:get-state',
  LOCALAI_RESET_PROBE: 'localai:reset-probe',
  LOCALAI_MODEL_CONSENT: 'localai:model-consent',
  LOCALAI_MODEL_DOWNLOAD_START: 'localai:model-download-start',
  LOCALAI_MODEL_DOWNLOAD_CANCEL: 'localai:model-download-cancel',
...
/** Main → renderer push: bundled-model download progress (LA7). */
export const LOCALAI_DOWNLOAD_PROGRESS_EVENT = 'localai:download-progress'

/**
 * Main → renderer push: the capture window is about to show. The pre-warmed
 * window is transparent and hidden between captures — visibilitychange is
 * unreliable there, so lock state went stale until reopen (QA2-07). Every
 * show pings the page to re-sync profile/lock state.
 */
...
export interface MossBridge {
  ...
  data: {
    getOverview: () => Promise<DataOverview>
    showInFolder: () => Promise<{ ok: true }>
    /** Native directory picker — resolves null when the operator cancels. */
    pickMoveTarget: () => Promise<string | null>
    /** Verified copy + flip + relaunch; on success MOSS restarts itself. */
    moveFolder: (targetPath: string) => Promise<MoveDataFolderResult>
  }
  profiles: {
  ...
  shell: {
    ...
    getAppSettings: () => Promise<import('@shared/appSettings').MossAppSettings>
    setAppSettings: (
      patch: Partial<import('@shared/appSettings').MossAppSettings>
    ) => Promise<import('@shared/appSettings').MossAppSettings>
  }
  money: {
  ...
  mail: {
    ...
    aiDraftReply: (messageId: string, instruction?: string) => Promise<MailAiDraftResult>
    listDrafts: (accountId?: string) => Promise<MailDraftSummary[]>
    getDraft: (draftId: string) => Promise<MailDraftRecord | null>
    saveDraft: (input: MailSaveDraftInput) => Promise<MailDraftRecord>
    deleteDraft: (draftId: string) => Promise<{ ok: true }>
    countDrafts: (accountId?: string) => Promise<number>
  }
  notes: {
    ...
    listAttachments: (noteId: string) => Promise<NoteAttachmentRecord[]>
    createAttachment: (input: CreateNoteAttachmentInput) => Promise<NoteAttachmentRecord>
    updateAttachment: (id: string, patch: UpdateNoteAttachmentInput) => Promise<NoteAttachmentRecord>
    updateSketch: (id: string, input: UpdateNoteSketchInput) => Promise<NoteAttachmentRecord>
  ...
  capture: {
    ...
    confirm: (input: CaptureCommitInput) => Promise<CaptureLoggedResult>
    hide: () => Promise<{ ok: true }>
    /** Fires every time the capture window shows — re-sync lock state (QA2-07). */
    onShown: (callback: () => void) => () => void
  }
  localai: {
    /** Parse-only: classifier → LLM fallback, returns a draft without writing. */
    describePreview: (text: string, surface?: LocalAiSurface) => Promise<CaptureSubmitResult>
    /** Warm-up ping on first focus of a Describe surface (plan §2 rule 5). */
    warm: () => Promise<{ ok: true }>
    getState: () => Promise<LocalAiPanelState>
    resetProbe: () => Promise<{ ok: true }>
    /** LA7 — consent for the one-time bundled-model download. */
    setModelConsent: (consent: 'accepted' | 'later') => Promise<{ ok: true }>
    startModelDownload: () => Promise<{ ok: true }>
    cancelModelDownload: () => Promise<{ ok: true }>
    onDownloadProgress: (callback: (state: LocalAiDownloadState) => void) => () => void
  }
  updates: {
