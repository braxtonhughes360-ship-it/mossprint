...
function computeStatus(
  ...
  rentGlance: Pick<MoneyRentGlance, 'configured' | 'covered' | 'label'>,
  overspendAtRisk: boolean
): { status: MoneyFlowStatus; label: string; why: string; whyAdvanced: string } {
  const housingOk = !rentGlance.configured || rentGlance.covered
  ...
  if (unassignedCents < 0 || forecastCents < 0 || !housingOk || !overspendCoverable) {
    return {
      ...
      whyAdvanced: !overspendCoverable
        ...
        : !housingOk && rentGlance.configured
          ? 'Housing envelope or bill underfunded.'
          : unassignedCents < 0
  ...
  if (overspentCount > 0 || overspendAtRisk || (forecastCents > 0 && forecastCents < 10000)) {
    return {
      status: 'tight',
      label: 'Tight',
      why:
        overspentCount > 0
          ...
          : overspendAtRisk
            ? 'An envelope is burning faster than the month.'
            : '',
      whyAdvanced:
        overspentCount > 0
          ? 'Envelope balance negative but coverable from ready-to-assign.'
          : overspendAtRisk
            ? 'Spend pace exceeds month progress on at least one envelope.'
  ...
  return {
    status: 'on_track',
    label: 'On track',
    why: unassignedCents === 0 ? 'Income is assigned and bills look covered.' : '',
    whyAdvanced: 'No overspend, housing covered, forecast non-negative.'
  }
}
...
function computeRentGlance(
  ...
  if (rentRow) {
    ...
    const covered =
      paidThisPeriod ||
      (rentRow.remainingCents >= 0 &&
        (need <= 0 || rentRow.remainingCents >= need || rentRow.remainingCents + safeToAssignCents >= need))
    ...
    const shortfall = Math.max(need - rentRow.remainingCents - safeToAssignCents, 0)
    return {
      cents: rentRow.remainingCents,
      configured: true,
      label,
...
export function computeMoneyFlowGuidance(input: MoneyFlowGuidanceInput): MoneyFlowGuidance {
  ...
  const overspendRisk = computeOverspendRisk(budget.categories, budget.periodKey, today)
