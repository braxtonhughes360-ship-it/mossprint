...
function resolveWeekdayDateKey(
  ...
  // Weeks are Monday-anchored (WEEKDAY_OFFSET: Mon=0).
  // 'this <weekday>'  = that weekday in the current Mon–Sun week — may already
  //   have passed; the quick-add flow shows an editable date, so never silently
  //   roll a passed "this" weekday into next week.
  // 'next <weekday>'  = the same slot in the following Mon–Sun week ("this" + 7).
  // bare '<weekday>'  = nearest upcoming occurrence (today counts).
  if (modifier === 'this') {
    return shiftDateKey(todayKey, targetOffset - todayOffset)
  }
  if (modifier === 'next') {
    return shiftDateKey(todayKey, targetOffset - todayOffset + 7)
  }
  return shiftDateKey(todayKey, (targetOffset - todayOffset + 7) % 7)
}
...
function extractTime(text: string): {
  ...
  // Word times first: "lunch tomorrow at noon" must not fall through to the
  // 9:00 default with "at noon" left dangling in the title.
  const wordTimeMatch = remainder.match(/\b(?:at\s+|@\s*)?(noon|midday|midnight)\b/i)
  if (wordTimeMatch) {
    hour = wordTimeMatch[1].toLowerCase() === 'midnight' ? 0 : 12
    remainder = remainder.replace(wordTimeMatch[0], ' ')
...
export const CALENDAR_PARSE_FIXTURES: Array<{
  ...
  {
    // Word time: "at noon" = 12:00 and must not leak into the title (beta.5 audit).
    input: 'lunch with Sam tomorrow at noon',
    weekStartKey: '2026-06-15',
    todayKey: '2026-06-19',
    expect: { title: 'lunch with Sam', dateKey: '2026-06-20', hour: 12, minute: 0 }
  },
  {
    input: 'flight check-in at midnight tomorrow',
    weekStartKey: '2026-06-15',
    todayKey: '2026-06-19',
    expect: { title: 'flight check-in', dateKey: '2026-06-20', hour: 0, minute: 0 }
  },
  {
    // today is Friday 06-19: "next saturday" = Saturday of the FOLLOWING
    // Mon–Sun week (06-27), not tomorrow — the A1 beta.4 semantics.
    input: 'going to beach next saturday at 6pm with lizzie',
    weekStartKey: '2026-06-15',
    todayKey: '2026-06-19',
    expect: { title: 'going to beach with lizzie', dateKey: '2026-06-27', hour: 18, minute: 0 }
  },
  {
    // Thursday anchor: "this friday" = tomorrow (current Mon–Sun week).
    input: 'coffee this friday at 8am',
    weekStartKey: '2026-06-15',
    todayKey: '2026-06-18',
    expect: { title: 'coffee', dateKey: '2026-06-19', hour: 8, minute: 0 }
  },
  {
    // Thursday anchor: "next friday" = Friday of the following week (+8),
    // NOT tomorrow — the exact repro that shipped wrong pre-beta.4.
    input: 'basketball next friday at 7pm',
    weekStartKey: '2026-06-15',
    todayKey: '2026-06-18',
    expect: { title: 'basketball', dateKey: '2026-06-26', hour: 19, minute: 0 }
  },
  {
    // Thursday anchor: "next monday" = next week's Monday.
    input: 'standup next monday at 9am',
    weekStartKey: '2026-06-15',
    todayKey: '2026-06-18',
    expect: { title: 'standup', dateKey: '2026-06-22', hour: 9, minute: 0 }
  },
  {
    // Thursday anchor: "this monday" already passed — keep the past date;
    // quick-add shows an editable date the user confirms.
    input: 'review this monday at 4pm',
    weekStartKey: '2026-06-15',
    todayKey: '2026-06-18',
    expect: { title: 'review', dateKey: '2026-06-15', hour: 16, minute: 0 }
  },
  {
