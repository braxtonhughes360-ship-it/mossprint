...
/**
 * Shared income detection for deterministic money shapes ("$1400 paycheck").
 * Capture's instant-write path uses this to route income with a confirm
 * instead of silently logging an expense (beta.5 A2).
 */
export function hasMoneyIncomeHint(text: string): boolean {
  return INCOME_HINT_PATTERN.test(text)
}

/** "<payee> <amount>" with the amount trailing — "coffee 4.50", "got paid 2,400". */
const TRAILING_AMOUNT_PATTERN = /^(.*\S)\s+\$?((?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d{1,2})?)$/
