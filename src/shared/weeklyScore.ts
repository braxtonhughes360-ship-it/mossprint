...
function computeGoalsPillar(goals: GoalWeekSnapshot, todayKey: string): WeeklyScorePillar {
  ...
  return {
    ...
    score,
    summary: trustworthy
      ? `${completed.length}/${scheduled.length} done`
      : 'Add a habit with days this week',
    detail: trustworthy
      ? `${completed.length} of ${scheduled.length} scheduled habit check-ins completed.`
      : 'Create at least one habit scheduled on a day that has already passed this week.'
  }
}
  ...
  return {
    ...
    summary: trustworthy
      ? active.length > 0
        ...
        : 'No spending yet'
      : 'Assign envelopes or log income',
    detail: trustworthy
      ? active.length > 0
        ? `${healthy.length} of ${active.length} active envelopes are not overspent.`
        : 'Budget is set up; no envelope activity yet this period.'
      : 'Record a paycheck or assign at least three envelopes to unlock the budget pillar.'
  }
}
...
function computeNutritionPillar(
  ...
  const daysNeeded = Math.max(0, NUTRITION_MIN_LOGGED_DAYS - logged.length)
  ...
  return {
    ...
    score,
    summary: trustworthy
      ? `${onTarget.length}/${logged.length} days on target`
      : daysNeeded > 0
    ...
    detail: trustworthy
      ...
      : daysNeeded > 0
        ? `Log food on ${daysNeeded} more elapsed day${daysNeeded === 1 ? '' : 's'} this week.`
        : 'Set a calorie target in Nutrition → Goals.'
  }
}
...
export function computeWeeklyScore(input: WeeklyScoreComputeInput): WeeklyScoreSnapshot {
  ...
  let headline = 'Insufficient data'
  let hint = 'Need at least two active pillars with real data this week.'

  if (status === 'ready' && score !== null) {
    headline = `Weekly score · ${score}`
    hint = `Equal blend of ${eligible.map((p) => p.label.toLowerCase()).join(', ')}.`
  } else {
    const missing = pillars.filter((p) => !p.trustworthy)
    if (missing.length > 0) {
      hint = missing.map((p) => p.summary).join(' · ')
    }
  }
