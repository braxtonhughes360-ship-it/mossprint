...
3. **OAuth consent screen** → External → fill app name "MOSS", your email
   - Add scope: `.../auth/calendar.events` (read **and** write — MOSS pushes events you
     create in MOSS to your Google calendar; `calendar.readonly` alone breaks that)
...
| Two-way | No — read-only | Yes — events created in MOSS appear in Google |
